...
class SearchOptions(BaseModel):
    ...
    request: Optional[conlist(StrictStr)] = Field(
        None, description="Gets the list of search request filters."
    )
    __properties = ["request"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> SearchOptions:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> SearchOptions:
        """Create an instance of SearchOptions from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return SearchOptions.parse_obj(obj)

        _obj = SearchOptions.parse_obj({"request": obj.get("request")})
        return _obj
